...
class VehicleBuyer(VehicleRenter):
    ...
    def _getPrice(self):
        return getCrewAndShellsSumPrice(self.vehicle.buyPrice, self.vehicle, self.crewType, self.buyShell)
...
def tryToLoadDefaultShellsLayout(vehicle, callback=None):
    ...
    if callback is not None:
        callback(False)
    return
# okay decompiling ./res/scripts/client/gui/shared/gui_items/processors/vehicle.pyc
