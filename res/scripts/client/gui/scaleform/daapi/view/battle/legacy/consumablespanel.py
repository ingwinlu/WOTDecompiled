...
# Embedded file name: scripts/client/gui/Scaleform/daapi/view/battle/legacy/ConsumablesPanel.py
...
from gui.Scaleform.daapi.view.battle.legacy import COMMAND_AMMO_CHOICE_MASK, AMMO_ICON_PATH, NO_AMMO_ICON_PATH
from gui.battle_control import g_sessionProvider
from gui.battle_control.battle_constants import VEHICLE_VIEW_STATE, VEHICLE_DEVICE_IN_COMPLEX_ITEM, GUN_RELOADING_VALUE_TYPE
...
class ConsumablesPanel(object):
    ...
    def start(self):
        ...
        if self.__flashObject:
            ...
            props = _FalloutSlotViewProps(useStandardLayout=not visitor.hasRage())
            self.__flashObject.setProperties(visitor.gui.isFalloutBattle(), props._asdict())
            self.__addListeners()
        else:
    ...
    def __onGunReloadTimeSet(self, currShellCD, state):
        if currShellCD in self.__cds:
            ...
            if valueType == GUN_RELOADING_VALUE_TYPE.TIME:
                self.__flashObject.setCoolDownTime(index, state.getActualValue())
            elif valueType == GUN_RELOADING_VALUE_TYPE.TIME:
                self.__flashObject.setCoolDownPosAsPercent(index, state.getActualValue())
        else:
...
class _RageBarPlugin(IPlugin):
    ...
    def start(self):
        ...
        self._parentObj.flashObject.initializeRageProgress(g_sessionProvider.arenaVisitor.gui.isFalloutBattle(), rageProps._asdict())
        return
...
# okay decompiling ./res/scripts/client/gui/scaleform/daapi/view/battle/legacy/consumablespanel.pyc
