...
class ShopBoostersTab(BoosterTab):
    ...
    def __getActionVO(booster):
        if booster.buyPrice != booster.defaultPrice:
            return packActionTooltipData(ACTION_TOOLTIPS_TYPE.BOOSTER, str(booster.boosterID), True, booster.buyPrice, booster.defaultPrice)
        else:
            return None
...
def _makeBoosterSlotVO(booster, count):
    return {'boosterId': booster.boosterID,
     ...
     'showLeftTime': False}
# okay decompiling ./res/scripts/client/gui/scaleform/daapi/view/lobby/boosters/booster_tabs.pyc
